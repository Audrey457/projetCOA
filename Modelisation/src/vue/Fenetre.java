...
public class Fenetre extends JFrame implements Observer {
	...
	private JButton donneesCSV, plugins, undo, redo, choixAffichTab, choixAffichCourbe, quit, choixNbCourbe, selectUrl;
	...
	private void initialiser() {
		...
		urlRessource = new JTextField(
				"http://real-chart.finance.yahoo.com/table.csv?s=%5EFCHI&d=0&e=18&f=2017&g=d&a=2&b=1&c=1990&ignore=.csv");
	...
	private void positionner() {
		...
		gauche.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createMatteBorder(0, 0, 0, 1, Color.BLACK),
				BorderFactory.createEmptyBorder(10, 10, 10, 10)));

		// boutons charger donnees depuis CSV, charger donnees depuis URL
		...
		hautGauche.add(urlRessource);
		hautGauche.add(this.selectUrl);
		hautGauche.add(Box.createRigidArea(new Dimension(0, 30)));
		...
		basGauche.add(plugins);
		basGauche.add(Box.createRigidArea(new Dimension(0, 20)));
		basGauche.add(new JLabel("Choisir opération :"));
		 ...
		 * panneau droit
		 */
		affich.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createEmptyBorder(),
				BorderFactory.createEmptyBorder(10, 10, 10, 10)));

		JScrollPane tab = new JScrollPane(vueTab);
		tab.setPreferredSize(new Dimension(1000, 500));
		cardAffichTab.add(tab);
		...
		this.add(gauche, BorderLayout.LINE_START);
		this.add(droit, BorderLayout.CENTER);
	}
	...
	private void choixOpeActionPerformed(ActionEvent e) throws Exception {
		...
		if (!this.plugsTrait.isEmpty() || !this.plugsTransfo.isEmpty()) {
			...
			if (clicIndex < taillePlugsTransfo) {
				...
				choixNbCourbe.setEnabled(true);
			} // on applique une operation qui genere un seul chiffre et on
				// affiche le resultat dans un pop up
			else {
